...
// 임시 비번 보내고 해당 회원의 실제 database 비번 값도 바꿔야 함.
// 해당이메일을 가진 회원이 있는 지 일단 찾아야 함.
router.post('/setpassword', async (req, res, next) => {
   ...
   try {
      ...
      transporter.sendMail(mailOptions, (error, info) => {
         if (error) {
            console.error(error) //에러 나면 아래 catch문으로 가지는지? 아니면 아래 트랜잭션 커밋이 실행돼서 망함
            throw new Error('에러 쓰로우 됐습니다! 선생님이 말씀하신대로 성공!')
         } else {
